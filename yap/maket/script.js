...
const itemWidth1 = document.querySelector('#slider-1 .slider-container').clientWidth; // Шаг прокрутки 1 слайдера
const itemWidth2 = document.querySelector('#slider-2 .slider-container').clientWidth; // Шаг прокрутки 2 слайдера
  ...
  if (b == 1) { // Если 1 слайдер
    for (let i = 0; i < ind1.length; i++) { // Перебор всех индикаторов слайдера
      if ((Math.abs(pos1) / itemWidth1) == i) { // Если номер показываемого элемента = номеру индикатора
        if (ind1[i].classList.contains('active') == false) ind1[i].classList.add('active'); // Добавить класс акт
      } else if (ind1[i].classList.contains('active')) ind1[i].classList.remove('active'); // Если нет, удалить
    }
  } else { // Если 2 слайдер
    for (let i = 0; i < ind2.length; i++) { // Перебор всех индикаторов слайдера
      if ((Math.abs(pos2) / itemWidth2) == i) { // Если номер показываемого элемента = номеру индикатора
        if (ind2[i].classList.contains('active') == false) ind2[i].classList.add('active'); // Добавить класс акт
      } else if (ind2[i].classList.contains('active')) ind2[i].classList.remove('active'); // Если нет, удалить
    }
  }
}
...
window.addEventListener('scroll', () => { // Событие при скроллинге страницы
  if (getTop() > offset) scrollUp.classList.add('active'); // Если проскроллено больше расстояния скролла - якорь акт
  else scrollUp.classList.remove('active'); // Если нет - нет
});
