...
function changeUiStyle(targetUI,uiParametresArray) { // Функция смены стиля UI
  ...
  if (headerStyleType != 'custom') { // Если активен был дефолтный набор рз, либо ключ пуст
    classSwith(styles[0],uiParametresArray[9][0]); // Изменить стиль диалоговых окон на дефолтный для данного UI
    markInput(checkedInputs[0][2],checkedInputs[1][2],uiParametresArray[8][0]); // Отметить нужн. инпут
    localStorage.setItem(styles[0][0],uiParametresArray[9][0]); // Сохранить ключ стиля диалоговых окон
  }
